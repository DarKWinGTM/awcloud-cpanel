...
import cloudscraper
import pytz
from flask import Flask, json, jsonify, request, redirect, send_from_directory, Response, make_response
...
def index():
    ...
    <head>
        <meta
            charset         = "utf-8"
        />
        <title>Control Panel - [XXX IP XXX]</title>
        <link
        ...
        <script type=text/javascript>
            ...
            s.setAttribute('src', '/js/contoller.js'); 
        ...
        <script type=text/javascript>
            var s   = document.createElement('script'); 
            s.setAttribute('type', 'text/javascript'); 
            s.setAttribute('src', '/js/notify.js'); 
            document.head.appendChild(s); 
        </script>
        ...
        >
            :root {
                --bs-font-sans-serif: system-ui !important;
                --bs-font-monospace: system-ui !important;
            }
            ...
            thead, tr, td, th {
                border: 5px solid rgb(141, 197, 63) !important;
                border-radius: 25px;
                color: #fff;
                height: 100%;
                max-height: 430px;
    ...
    <body>
        <div class="d-flex justify-content-center" id="FlowChart" style="width: 100%;height: 0%;">
        </div>
        
        <table cellpadding="0" class="table table-bordered table-striped table-responsive table-striped" id="d_table" style="zoom: 0.8; width: 100%; ">
            <thead>
                <th colspan="8" style="
                    background-color    : rgb(141, 197, 63) !important;
                    color               : black;
                    border              : 0px !important;
                    border-radius       : 1px !important;
                    font-family         : sans-serif;
                    font-size           : large;
                "><div class="d-flex justify-content-center" style="">T.XXXXX = TIME | P.TLM = TLM Recieved previous mine | P.CPU = CPU Usage previous mine</div></th>
                <tr>
                    <th scope="col"> # WAXID </th>
                    <th class="col-md-2" scope="col">CPU</th>
                    <th scope="col">STAKE</th>
                    <th class="col-md-2" scope="col">TLM / WAX</th>
                    <th scope="col">LAND</th>
                    <th class="col-md-2" scope="col">T.DELAY : T.STAMP</th>
                    <th class="col-md-2" scope="col">P.TLM : P.CPU</th>
                    <th class="col-md-2" scope="col">STATE</th>
                </tr>
            </thead>
        </table>
        
        <div class="progress justify-content-center" style="width: 100%;height: 14px;text-align: center;margin: auto;bottom: 12px;position: relative;">
            <div
                class           = "progress-bar"
                role            = "progressbar"
                ...
                id              = "time_pg"
                aria-valuenow   = "0"
                description     = "https://jsfiddle.net/bozdoz/fSLdG/2/"
            ></div>
        </div>
        
        <div class="container d-flex justify-content-center" style="zoom: 0.8; " >
            <div class="row row-cols-auto">
                <form action="#ADD" onsubmit="return false" class="row row-cols-auto needs-validation" validate>
                    <div class="col">
                        <input type="text" class="form-control" placeholder="TOKEN wYeaMc..." aria-label="ADD TOKEN" required>
                    </div>
                    <div class="col">
                        <input type="text" class="form-control" placeholder="WAXID xxxxx.wam" aria-label="ADD WAXID" required>
                    </div>
                    <div class="col">
                        <button type="submit" class="btn btn-primary add">ADD</button>
                    </div>
                </form>
                <form action="#DEL" onsubmit="return false" class="row row-cols-auto needs-validation" validate>
                    <div class="col">
                        <div class="input-group has-validation">
                            <input type="text" class="form-control" placeholder="WAXID xxxxx.wam" aria-label="DEL WAXID" required>
                        </div>
                    </div>
                    <div class="col">
                        <button type="submit" class="btn btn-primary del">DEL</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div class="container d-flex justify-content-center" style="zoom: 0.8; ">
            <div class="row row-cols-auto">
                <div class="row row-cols-auto">
                    <div class="col">
                        <button type="submit" class="btn btn-primary run">START</button>
                    </div>
                </div>
                <div class="row row-cols-auto">
                    <div class="col">
                        <button type="submit" class="btn btn-primary off">BREAK</button>
                    </div>
                </div>
            </div>
        </div>
...
</html>         '''.replace('XXX IP XXX', requests.get('https://checkip.amazonaws.com').text.strip())
    else:
        return jsonify(
            DATA = DATABASE, 
        ...
        for x in self.DATA['run']['content']:
            try:
                ...
                cloudscraper.create_scraper().post(
                    f'https://awcloud-data.patiwatnumbut.repl.co/del', 
                    json = {"waxid" : x['waxid']}
                )
                pass
            except Exception as e:

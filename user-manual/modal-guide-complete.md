...
1. **เลือก Source Account**
   ```
   From Account: masteraccount (Balance: 1000.0000 WAX)
   ```
...
3. **ตั้งค่าการแจกจ่าย**
   ```
   Total Amount: 300.0000 WAX
   Distribution Type: Equal (100 WAX per account)
   ```
...
1. **เลือกการแลกเปลี่ยน**
   ...
   From: TLM
   To: WAX
   Current Rate: 1 TLM = 0.02 WAX
   ```

2. **ใส่จำนวน**
   ```
   Amount: 1000.0000 TLM
   You'll receive: ~20.0000 WAX
   ```
...
**ตัวอย่างการตั้งค่า:**
```
Mining Delay: 1800000 (ms) = 30 minutes
Retry Attempts: 3
Fail Threshold: 5
```
...
1. **เลือก Account ที่ต้องการวิเคราะห์**
   ```
   Target Account: miningaccount1
